...
def generate_random_sentence():
    ...
    completion = client.chat.completions.create(
      model="gpt-4o-mini",
      messages=[
      {"role": "system", "content": "You are a language teacher. your job is to generate a small sentence in Hindi"},
      {"role": "user", "content": " Please generate a small sentence in hindi"}
      ]
    )
    ...
    if st.session_state.generated_sentence:
        ...
        if st.button('Verify Translation'):
            if user_translation:
                ...
                correction = verify_translation(st.session_state.generated_sentence, st.session_state.translation_input)
